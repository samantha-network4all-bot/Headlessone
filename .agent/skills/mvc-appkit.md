---
name: headlessone-mvc-appkit
description: Headlessone's architectural contract — every feature lives in an NSViewController that owns its model, its view, AND its HTTP test-API routes. Use this skill on every code-writing turn for the Headlessone project.
---

# Headlessone MVC for macOS / AppKit

This skill is **mandatory** for every code change to the Headlessone
...
- Plain Swift structs / classes in `Headlessone/<Feature>/<Name>State.swift`.
...
- An `NSView` subclass in `Headlessone/<Feature>/<Name>View.swift`.
...
- An `NSViewController` subclass in
  `Headlessone/<Feature>/<Name>Controller.swift`.
...
This is the single most important contract in Headlessone. Every
...
1. A route registered outside its controller. (e.g. `POST /editor/type`
   declared in `Headlessone/App/TestAPI/Routes.swift`.)
