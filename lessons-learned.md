...
- **Invariant**: every Headlessone target ships an explicit `main.swift`
...
- **Invariant**: every `NSTextView` in Headlessone is built by the canonical
...
- **Invariant**: Headlessone ships `NSOpenPanel` / `NSSavePanel` first.
...
- **Invariant**: in Headlessone the build check MUST invoke at least one
...
- **Invariant**: Headlessone's feature checks talk to the app via
...
- **Invariant**: each agent invocation in Headlessone is wrapped by Go
...
- **Invariant**: every code-agent invocation in Headlessone is sandwiched
...
- **Invariant**: in Headlessone, no quality review starts until the
...
- **Always commit on a working tree.** The old loop refused to
  ...
  Headlessone's orchestrator `git status --porcelain` checks both
  pre- and post-agent.
